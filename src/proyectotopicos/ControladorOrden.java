...
public class ControladorOrden {
    ...
    public void guardar(PanelPrincipal panel) {
        try {
            ...
            nuevaOrden.setHoraEntrada(sdfHora.format(new java.util.Date()));
            ...
            String estadoInicial = panel.jComboBox1.getSelectedItem().toString();
            modelo.addRow(new Object[]{idOrden, idCliente + " - " + nombreCliente, idVehiculo + " - " + descVehiculo, idServicio, fechaIngreso, costoFinal, estadoInicial});
    ...
    public void finalizarServicio(PanelPrincipal panel) {
        int fila = panel.jTable1.getSelectedRow();
        if (fila == -1) {
            JOptionPane.showMessageDialog(panel, "Selecciona una orden de la tabla.", "Aviso", JOptionPane.WARNING_MESSAGE);
            return;
        }
        DefaultTableModel modelo = (DefaultTableModel) panel.jTable1.getModel();
        ...
        for (Orden o : listaOrden) {
            if (o.getIdOrden().equals(idOrden)) {
                java.text.SimpleDateFormat sdfHora = new java.text.SimpleDateFormat("HH:mm:ss");
                String horaSalida = sdfHora.format(new java.util.Date());
                o.setHoraSalida(horaSalida);

                try {
                    java.time.LocalTime entrada = java.time.LocalTime.parse(o.getHoraEntrada());
                    java.time.LocalTime salida = java.time.LocalTime.parse(horaSalida);
                    long minutos = java.time.Duration.between(entrada, salida).toMinutes();
                    o.setDuracionMinutos(Math.max(minutos, 0));
                } catch (Exception e) {
                    o.setDuracionMinutos(0);
                }

                JOptionPane.showMessageDialog(panel,
                        "Servicio finalizado.\nDuración: " + o.getDuracionFormateada(),
    ...
    public void actualizar(PanelPrincipal panel) {
        try {
            ...
            modelo.setValueAt(panel.jComboBox1.getSelectedItem().toString(), fila, 6);
